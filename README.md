...
# Optional: probe a few hits for truthy trust_remote_code in tokenizer/config JSON (extra downloads)
"$(pwd)/.venv/bin/python" scripts/hub_find_models_under_size.py --max-mb 200 --per-query 10 --probe-trust-remote-code --max-probes 5
```
...
| [`scripts/hub_find_models_under_size.py`](scripts/hub_find_models_under_size.py) | Hub **metadata** search: repos whose summed file sizes are under **`--max-mb`** (default **200**) | `.venv/bin/python scripts/hub_find_models_under_size.py --max-mb 200 --per-query 12` |

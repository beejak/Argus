...
| **Orchestrator (phase 4)** | [`scripts/run_orchestrator_job.py`](scripts/run_orchestrator_job.py), [`docs/adr/0001-bundle-scanner-vs-orchestrator-scope.md`](docs/adr/0001-bundle-scanner-vs-orchestrator-scope.md) | Declarative job JSON → `scan-bundle` + optional **`dynamic_probe`** step + **envelope v2** (`run_id`, per-step timestamps). **`make orchestrator-validate`**. |
| **Dynamic probe stub (phase 5)** | [`scripts/run_dynamic_probe.py`](scripts/run_dynamic_probe.py), [`docs/PHASE5_DYNAMIC_PROBES.md`](docs/PHASE5_DYNAMIC_PROBES.md) | Opt-in via **`LLM_SCANNER_DYNAMIC_PROBE=1`**; writes **`llm_scanner.dynamic_probe_report.v1`**. Orchestrator **`run`** can chain the probe after **`scan_bundle`**, forwarding **`run_id`**, optional budgets, and optional config metadata (`garak_config`, `model_target`, required secret env-var names). **`make dynamic-probe-stub`**. |

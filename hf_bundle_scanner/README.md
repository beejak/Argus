...
- **CLI:** [`../scripts/run_dynamic_probe.py`](../scripts/run_dynamic_probe.py) — supports `--run-id`, budget flags, optional config/target/secret-name metadata (`--garak-config`, `--model-target`, `--secret-env-vars`), and controlled execution modes **`preflight`** (`garak --help`), **`selfcheck`** (`garak --version`), **`execute_once`** (non-empty `--execute-args`). From repo root: **`make dynamic-probe-stub`** writes `.agent/dynamic_probe_last.json` (disabled unless **`LLM_SCANNER_DYNAMIC_PROBE=1`**). Live targets (**`make dynamic-probe-live-*`**) prepend **`.venv-garak/bin`** to `PATH` so the main `.venv` stays CI-sized.

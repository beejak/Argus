...
- **CLI:** [`scripts/run_dynamic_probe.py`](../scripts/run_dynamic_probe.py) writes JSON to **`--out`**.
  - Optional flags: **`--run-id`**, **`--budget-max-probes`**, **`--budget-timeout-seconds`**.
...
- **Top-level `dynamic_probe` object:** non-empty **`out`** path for `llm_scanner.dynamic_probe_report.v1` JSON (written by [`scripts/run_dynamic_probe.py`](../scripts/run_dynamic_probe.py)); optional **`budget_max_probes`** and **`budget_timeout_seconds`** are validated as positive integers.
- **Runner:** [`scripts/run_orchestrator_job.py`](../scripts/run_orchestrator_job.py) `run` invokes the probe script after a successful job validation, forwards `run_id` + budget flags, merges **`aggregate_exit_code`** with the probe report’s **`exit_code`** via the same **worst-of** priority as bundle aggregates (`4 > 2 > 1 > 0`), and inserts a **middle** row in envelope **`llm_scanner.orchestrator_envelope.v2`** `steps[]`.
...
- Real Garak / PyRIT **job configs**, model targets, and rate-limit controls beyond the current `garak --help` stub.

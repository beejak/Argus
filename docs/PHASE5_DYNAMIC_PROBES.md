...
- **CLI:** [`scripts/run_dynamic_probe.py`](../scripts/run_dynamic_probe.py) writes JSON to **`--out`**.
  - Optional flags: **`--run-id`**, **`--budget-max-probes`**, **`--budget-timeout-seconds`**, **`--garak-config`**, **`--model-target`**, **`--secret-env-vars`**, **`--execution-mode`**, **`--execute-args`**.
  - Execution modes: `preflight` (default availability check) and controlled `execute_once` (one explicit garak argv payload, no shell).
  ...
  - `LLM_SCANNER_DYNAMIC_PROBE=1` and **`garak` present**:
    - `execution_mode=preflight` → runs bounded preflight (`garak --help`)
    - `execution_mode=execute_once` + non-empty `--execute-args` → runs one bounded garak command
    - on success → `status: ok`, exit `0`; on failure → `status: error`, exit `2`.
...
- Real Garak / PyRIT **probe execution breadth** beyond current preflight + execute-once controls.
